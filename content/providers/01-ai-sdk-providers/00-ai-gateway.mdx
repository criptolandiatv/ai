...
The Parallel Search tool enables models to search the web using [Parallel AI's Search API](https://docs.parallel.ai/api-reference/search/search-endpoint). This tool is optimized for LLM consumption, returning relevant excerpts from web pages that can replace multiple keyword searches with a single call.
...
- **searchType** _'list' | 'general' | 'single_page' | 'targeted'_

  Controls the depth vs breadth tradeoff:

  - `'list'` - Broad search returning up to 20 results with 1500 characters each (default)
  - `'general'` - Balanced search returning up to 10 results with 9000 characters each
  - `'targeted'` - Specific sources returning up to 5 results with 16000 characters each
  - `'single_page'` - Deep extraction returning up to 2 results with 30000 characters each

- **includeDomains** _string[]_

  List of domains to restrict search results to. Example: `['wikipedia.org', 'nature.com']`. Maximum 10 entries.

- **maxResults** _number_

  Maximum number of results to return. When not specified, defaults based on `searchType`.

- **maxCharsPerResult** _number_

  Maximum characters per result. When not specified, defaults based on `searchType`.
